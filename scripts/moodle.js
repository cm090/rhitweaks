    ...
    document.querySelectorAll('.block-timeline').forEach(timeline => {
        document.querySelectorAll('.timeline-event-list-item small.text-right').forEach(item => {
            ...
            if (additionalData.timeFormat == 12) {
                ...
                const half
                    = item.innerText.split(' ')[1] === "AM" ? time[0] === "12" ? -12 : 0 : 12;
                item.innerText
                    = `${String(parseInt(time[0]) + half).padStart(2, '0')}:${time[1]}`;
            }
        });
    });
    return Promise.resolve();
...
const updateCourseDropdown = () => {
    ...
    document.querySelector("#page-wrapper > nav > div.primary-navigation > nav")
...
const start = () => {
    ...
    modifyURL().then(() => {
        ...
        const wait = () => {
            if (document
                .querySelector('.block_timeline [data-region="event-list-loading-placeholder"].hidden')) {
        ...
        const addNavItems = () => {
            document.querySelectorAll('.dashboard-card-deck .dashboard-card, .list-group .course-listitem')
                .forEach(card => {
